...
def list_interview_sessions(user_id: int, limit: int, resume_id: int | None = None) -> list[dict]:
    settings = resolve_settings()
    repository = build_interview_session_repository(settings)
    return list_interview_sessions_service(user_id, limit, repository, resume_id=resume_id)
